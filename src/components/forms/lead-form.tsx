...
export function LeadForm({
  ...
  if (status === "ok") {
    return (
      <div className="surface-card">
        <p className="font-heading text-2xl text-fg">Request received</p>
  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-5">
      <div
        role="alert"
        className="rounded-lg border border-warning/40 bg-warning/10 px-4 py-3 text-sm leading-relaxed text-fg"
      >
        <strong className="font-semibold text-warning">No PHI on this form.</strong> Tell us the
      ...
      <div className="flex gap-2" aria-label="Form progress">
        {[1, 2, 3].map((n) => (
          <span key={n} className={cn("h-1 flex-1 rounded-full", step >= n ? "bg-primary" : "bg-surface-3")} />
        ))}
      </div>
      ...
      {step === 1 ? (
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="grid gap-1.5 sm:col-span-2">
            <p className="text-sm font-medium text-fg">Step 1 — Who should we reply to?</p>
          </div>
      ...
      {step === 2 ? (
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="grid gap-1.5 sm:col-span-2">
            <p className="text-sm font-medium text-fg">Step 2 — Practice context (optional fields welcome)</p>
          </div>
          <div className="grid gap-1.5">
      ...
      {step === 3 ? (
        <div className="grid gap-4">
          <div className="grid gap-1.5">
            <p className="text-sm font-medium text-fg">Step 3 — What is stuck in the cycle?</p>
